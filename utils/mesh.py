...
def meshDecimator(vertices, faces, target, remesh = False, optimalPlacement = True):
    startingVerticesShape = vertices.shape
    startingFacesShape = faces.shape
    ...
    print(f"Decimating Mesh:\nVertices = {startingVerticesShape} ==> {vertices.shape}\nFaces = {startingFacesShape} ==> {faces.shape}")
...
def meshCleaner(vertices, faces, vPct = 1, minF = 8, minD = 5, repair = True, remesh = True, remeshSize = 0.1):
    ...
    if minD > 0:
        meshSet.meshing_remove_connected_component_by_diameter(mincompomentdiag = pml.Percentage(minD))
    if minF > 0:
        meshSet.meshing_remove_connected_component_by_face_number(mincompomentsize = minF)
    if repair:
