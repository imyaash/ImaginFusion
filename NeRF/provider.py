...
from utils.functions import circlePoses, randomPoses, getRays
...
class Dataset:
    ...
    def collate(self, index):
        B = len(index)
        if self.training:
            poses, dirs, thetas, phis, radius = randomPoses(
                B, self.device, self.args, radiusRange = self.args.radiusRange,
                thetaRange = self.args.thetaRange, phiRange = self.args.phiRange,
                returnDirs = True, angleOverhead = self.args.angleOverhead,
                angleFront = self.args.angleFront, uniformSphereRate = self.args.uniformSphereRate
            )
            fov = random.random() * (
    ...
    def dataLoader(self, batchSize = None):
        ...
        loader = DataLoader(
            list(range(self.size)), batch_size = batchSize, collate_fn = self.collate, shuffle = self.training, num_workers = 0
        )
        loader._data = self
        return loader