...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        predMask = outputs["weightsSum"].reshape(B, 1, H, W)
        predNormal = outputs["normalImage"].reshape(B, H, W, 3) if "normalImage" in outputs else None
        predRGB = torch.cat([outputs["image"], outputs["weightsSum"].unsqueeze(-1)], dim = -1).reshape(B, H, W, 4).permute(0, 3, 1, 2).contiguous() if asLatent else outputs["image"].reshape(B, H, W, 3).permute(0, 3, 1, 2).contiguous()
    ...
    def eval_step(self, data):
        ...
        ambientRatio = data["ambientRatio"] if "ambientRatio" in data else 1.0
        lightD = data["lightD"] if "lightD" in data else None
